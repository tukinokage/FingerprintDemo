...
class FingerprintManager{
    companion object{
        ...
        val executor by lazy(mode = LazyThreadSafetyMode.SYNCHRONIZED) { Executors.newSingleThreadExecutor() }
        @JvmStatic
        /**@param initPurpose KeyProperties.PURPOSE_ENCRYPT or KeyProperties.PURPOSE_DECRYPT*/
        fun showBiometricPrompDialog(initPurpose :Int){
            ...
            val enCryptedCallback =object : BiometricPrompt.AuthenticationCallback() {//不同系统
            ...
            val deCryptedCallback = object : BiometricPrompt.AuthenticationCallback() {
                override fun onAuthenticationSucceeded(result: BiometricPrompt.AuthenticationResult?) {
                    ...
                    if(doFinal == null){
                        Log.d(this@Companion.javaClass.simpleName, "the secret is n-one")
                    }else{
                        Log.d(this@Companion.javaClass.simpleName, "secret is "+String(doFinal))
                    }
                ...
                override fun onAuthenticationFailed() {
                    super.onAuthenticationFailed()
                    Log.e(this.javaClass.simpleName, "fingerPrintFailed")
                }
            ...
            when(initPurpose){
                KeyProperties.PURPOSE_DECRYPT ->{
                    authenticationFingerprint(KeyProperties.PURPOSE_DECRYPT, deCryptedCallback, null)
                }
                KeyProperties.PURPOSE_ENCRYPT ->{
                    authenticationFingerprint( KeyProperties.PURPOSE_ENCRYPT, enCryptedCallback, SharedPrenfencesUtil.spInstance.getString(context, paySecretIv))
                }
            }
        }
...
//            KeyGenParameterSpec.Builder("TEST", initPurpose).build()
        ...
        fun checkIsSupport(context: Context):FingerPrintInitType{
            ...
            } else if (!BiometricPrompt.BIOMETRIC_ERROR_HW_NOT_PRESENT){
